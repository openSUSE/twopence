...
int main(int argc, char *argv[])
{
  ...
  inject_file = get_function(dl_handle, "twopence_inject_file");
  ...
  // Inject file
  rc = (*inject_file)(twopence_handle, opt_user, opt_local, opt_remote,
                      &remote_error, true);
  if (rc == 0) printf("File successfully injected\n");
